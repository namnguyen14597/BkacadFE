...
const Class = () => {
    ...
    const handleUpdate = () => {
        if (!editField) return;
        apiLoggedInInstance({
            url: "/api/class/" + editField.id,
            params: {
                course_id: editField.courseId,
                name: editField.name
            },
            method: "PUT"
        }).then(() => {
    ...
    const handleSearch = () => {
        apiLoggedInInstance({
            ...
            filteredData = filteredData.filter(item => {
                const courseIdString = String(item.courseId); // Chuyển đổi item.year thành chuỗi
                return courseIdString.includes(filterValue.courseId) && item.name.includes(filterValue.name);
            });
    ...
    return (
        <div className='w-full'>
            <div className="w-full flex justify-between items-center px-14 py-3 shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
                <div className='font-medium'>Khóa ({data.length})</div>
                <div className="">
            ...
            <div className='relative overflow-x-auto mx-4'>
                <table className='border border-[#F0F0F0] w-full  mt-10'>
                    ...
                    <tbody className=''>
                        <tr className='border-b '>
                            ...
                            <td>
                                <input value={filterValue.courseId} onChange={(event) => {
                                    setFilterValue(pre => ({ ...pre, courseId: event.target.value }))
                        ...
                        {paginatedData.map((field, index) => (
                            <tr className='bg-white border-b dark:bg-gray-800 dark:border-gray-700 items-center'>
                                ...
                                <td className='px-1 py-4'>{field.courseId}</td>
                ...
                {showModal && (<Modal title={editField ? "Chỉnh sửa khóa" : "Thêm khóa"} onClose={handleClose}>
                    {editField ? (
                        <div>
                            <div className='flex justify-between gap-[100px] px-[30px] mb-5'>
                                <div className='w-1/2'>
                                    <p className='flex justify-start mb-2 font-semibold'>Tên khóa</p>
                                    <input value={editField.courseId} onChange={(e) => {
                                        setEditField(pre => ({ ...pre, courseId: e.target.value }))
                                    }} className='border w-full p-2' placeholder='Tên khóa' />
                                </div>
                                <div className='w-1/2'>
                                    <p className='flex justify-start mb-2 font-semibold'>Năm bắt đầu</p>
                                    <input value={editField.name} onChange={(e) => {
                                        setEditField(pre => ({ ...pre, name: e.target.value }))
                                    }} autoFocus className='w-full border p-2' placeholder='Năm bắt đầu' />
                                </div>
                            </div>
                        ...
                        <div>
                            <div className='flex justify-between gap-[100px] px-[30px] mb-5'>
                                <div className='w-1/2'>
                                    ...
                                    }} autoFocus className='w-full border p-2' placeholder='Tên khóa' />
                                </div>
                                <div className='w-1/2'>
                                    <p className='flex justify-start mb-2 font-semibold'>Năm bắt đầu</p>
                                    <input value={name} onChange={(e) => {
                                        setName(e.target.value)
                                    }} className='w-full border p-2' placeholder='Năm bắt đầu' />
                                </div>
                            </div>
                            <div className='border-t flex justify-end pr-[30px]'>
                                <button onClick={() => {
